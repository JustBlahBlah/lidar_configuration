...
enum class Output_format { UNDIFINED, CM } of = Output_format::UNDIFINED;

static void printHelp() {
  std::cout
      << "Lidar Configuration Tool\n"
      << "Usage: ./<path/to/config>config [options]\n\n"
      << "Options:\n"
      << "  -p, --port <name>          Specify serial port (e.g. COM3 or "
         "/dev/ttyUSB0)\n"
...
int main(int argc, char **argv) {
  ...
  // Используем порт по умолчанию из defines.hpp (COM3 для Win, /dev/ttyUSB0 для
  // Linux)
  ...
  if (current_baud_rate == 0) {
    std::cerr << "[ERROR] You must specify current baud rate with -c <baud>"
              << std::endl;
    return 1;
  }
  ...
  if (check_strict_connection(serial_fd)) {
    std::cout << "Connection confirmed at " << current_baud_rate << " baud."
              << std::endl;
  } else {
    std::cerr << " [ERROR] LiDAR not found at " << current_baud_rate << " baud."
              << std::endl;
    close_port(serial_fd);
    return 1;
  }
    ...
    case Commands::BAUD_RATE:
      ...
      case Baud_rate::_BAUD_115200:
        if (current_baud_rate != 115200) {
          ...
          send_command(serial_fd, "Set Baud 115200",
                       {0x5A, 0x08, 0x06, 0x00, 0xC2, 0x01, 0x00});
      ...
      case Baud_rate::_BAUD_256000:
        if (current_baud_rate != 256000) {
          ...
          send_command(serial_fd, "Set Baud 256000",
                       {0x5A, 0x08, 0x06, 0x00, 0xE8, 0x03, 0x00});
          send_command(serial_fd, "Save Settings", {0x5A, 0x04, 0x11, 0x6F});
